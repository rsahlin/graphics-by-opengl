...
public abstract class VulkanWrapper extends Backend {
	...
	protected void initVulkanBackend() {
		...
		String[] folders = FileUtils.getInstance().listResourceFolders(path);
		ArrayList<String> filenames = FileUtils.getInstance().listFiles(path, folders, ".vert");
		for (String name : filenames) {
			SimpleLogger.d(getClass(), "Found file: " + name);
		}
	}
