...
/**
 * A pipeline holds the data that is needed for the processing stages. Some of
 * this data is immutable, such as shader and blend function
 *
 */
public class GLPipeline implements GraphicsPipeline<GLShaderSource> {
    ...
    /**
     * Block variables used in the compiled program - key is the uniform index from
     * GL
     */
    protected HashMap<Integer, NamedShaderVariable> blockVariables = new HashMap<>();
    /**
     * Calculated in create program, created using {@link #attributeBufferCount} If
     * attributes are dynamically mapped (not using indexer) then only one buffer is
     * used.
     */
    transient protected ShaderVariable[][] attributeVariables;
    /**
     * The size of each buffer for the attribute variables - as set either from
     * indexer if this is used or taken from defined attributes.
     */
    protected int[] attributesPerVertex;
    /**
     * Optional additional storage per vertex, used when attribute buffer is
     * created.
     */
    ...
    public void update(NucleusRenderer renderer, GLTF gltf, Primitive primitive, float[][] matrices)
        ...
        if (material != null) {
            ...
            // ((GLTFShaderProgram) shader).prepareTextures(renderer, gltf, primitive,
            // material);
    ...
    /**
     * Sets the name of the shaders in this program and returns an array of
     * {@link ShaderSource}, normally 2 - one for vertex and one for
     * fragment-shader. This method must be called before the program is created.
     * 
    ...
    protected GLShaderSource[] createShaderSource(Renderers version, Categorizer function, ProgramType type) {
        ...
        switch (type) {
            ...
            case VERTEX_GEOMETRY_FRAGMENT:
                ...
                shaderTypes = new ShaderType[] { ShaderType.VERTEX, ShaderType.GEOMETRY, ShaderType.FRAGMENT };
                break;
            default:
    ...
    public GLShaderSource getShaderSource(Renderers version, Categorizer function, ShaderType type) {
        ...
        switch (type) {
            case VERTEX:
                return new GLShaderSource(ShaderBinary.PROGRAM_DIRECTORY + sourceNameVersion,
                        function.getShaderSourceName(type), function, VERTEX_TYPE_SUFFIX, type);
            case FRAGMENT:
                return new GLShaderSource(ShaderBinary.PROGRAM_DIRECTORY + sourceNameVersion,
                        function.getShaderSourceName(type), function, FRAGMENT_TYPE_SUFFIX, type);
            case COMPUTE:
                return new GLShaderSource(ShaderBinary.PROGRAM_DIRECTORY + sourceNameVersion,
                        function.getShaderSourceName(type), function, COMPUTE_TYPE_SUFFIX, type);
            case GEOMETRY:
                return new GLShaderSource(ShaderBinary.PROGRAM_DIRECTORY + sourceNameVersion,
                        function.getShaderSourceName(type), function, GEOMETRY_TYPE_SUFFIX, type);
            default:
    ...
    public void createProgram(NucleusRenderer renderer, GLShaderSource[] sources, GraphicsShader shader)
            throws BackendException {
        SimpleLogger.d(getClass(), "Creating program for: " + sources.length + " shaders in program "
                + getClass().getSimpleName() + ", sources are:");
        ...
        try {
            ...
            for (int shaderIndex = 0; shaderIndex < sources.length; shaderIndex++) {
                ...
                SimpleLogger.d(getClass(), "Compiling " + sources[shaderIndex].getFullSourceName());
    ...
    /**
     * Checks the compile status of the specified shader program - if shader is not
     * successfully compiled an exception is thrown.
     * 
     * @param gles GLES20 platform specific wrapper.
     * @param source
     * @param shader
    ...
    /**
     * Fetches the program info and stores in this class. This will read active
     * attribute and uniform names, call this after the program has been compiled
     * and linked.
     * 
     * @param gles
    ...
    /**
     * Checks the link status of the specified program - if link status returns
    ...
    /**
     * Validates the program - only call this in debug mode. Set uniform and texture
     ...
     */
    private void linkProgram(GLES20Wrapper gles, int program, int[] shaderNames) throws GLException {
    ...
    /**
     * Fetches active variables info of the specified type (attribute or uniform)
     * and calls implementing shader program class to store the shader variable.
     * This will create the mapping between the specific shader program and the
     * location of shader variables so that each shader variable can be indexed
     * using the implementing shader program.
     * 
     * @param gles
     * @param type Type of variable to fetch info for.
     * @param info
     * @param block Variable block info or null - store block variables here
     * @throws GLException If attribute or uniform location(s) are -1, ie they could
     * not be found using the name.
     */
    private void fetchActiveVariables(GLES20Wrapper gles, VariableType type, ProgramInfo info, InterfaceBlock block)
        ...
        VariableType infoType = type != ShaderVariable.VariableType.UNIFORM_BLOCK ? type : VariableType.UNIFORM;
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (block != null) {
                ...
                // Check if uniform variable already has been fetched from block - then check
                // that it is used in type of
                // shader.
    ...
    /**
     * Returns ShaderVariable from variable blocks, if the block variable is used in
     * the type.
     * 
    ...
    /**
     * Stores the shader variable in this program, if variable is of unmapped type,
     * for instance Sampler, then it is skipped. Also skip variables that are
     * defined in code but not used in shader. Variables are stored in
     * {@link #activeUniforms} or {@link #activeAttributes}
     * 
     * @param variable
     * @throws IllegalArgumentException If shader variables are null, the program
     * has probably not been created, or if a
     * variable has no mapping in the code.
     */
    ...
    /**
     * Sets the active shader variable into {@link NamedShaderVariable} array - call
     * this when variable has been validated.
     * 
     * @param variable
     */
    ...
    /**
     * Maps shader variables to defined offsets or updates the shader variable
     * offsets to runtime values
     * 
     * @param gles
     * @indexer Offsets to use for shader variables, if specified active variables
     * will be mapped to offsets as found when comparing variable name. If
     * null then offsets will be updated according to used variables in an
     * increasing manner and will be tightly packed.
     * @throws GLException
     */
    ...
    /**
     * Use the offset as specified in the indexer and update or set the offset in
     * program variables. Use this when the offset mapping of variables shall be
     * controlled, for instance by a shared program.
     * 
     * @param gles
     * @param indexer
     */
     ...
     * The offset will be tightly packed based on used variable size, the order of
     * used variables will be the same.
     */
     ...
     * Always creates a {@link FloatBlockBuffer} for uniforms. If BlockBuffer is
     * needed for other type than uniforms it needs to be implemented. Binds uniform
     * block to a binding point using the block index, this means that there is one
     * binding point per uniform block.
     * 
     * @param renderer
     * @return Uniform variable block buffers, using buffer objects, for this
     * program, or null if not used.
     */
    ...
    /**
     * returns the size, in basic machine units, of all active variables in the
     * block.
     * 
     * @param block
     * @return
    ...
    /**
     * Align the size of the variables (per vertex) of the type in the buffer with
     * index, override in subclasses if for instance Attributes shall be aligned to
     * a specific size, eg vec4
     * 
    ...
    /**
     * Sorts the attributes used based on BufferIndex - attribute variables are
     * sorted based on buffer in the specified result array. Finds the shader
     * attribute variables per buffer using VariableMapping, iterate through defined
     * (by subclasses) attribute variable mapping. Put the result in the result
     * array and set the {@linkplain NamedShaderVariable} offset based on used
     * attributes.
     * 
     * TODO Add check for mismatch of size, ie if ShaderVariables has one variable
     * as float3 and it is defined is program as float4 then raise error.
     * 
     * @param resultArray Array to store shader variables for each attribute buffer
     * in, attributes for buffer 1 will go at index 0.
     */
    ...
    /**
     * Sort the variables belonging to the specified buffer index. Returning an
     ...
     * @return
     */
    protected ShaderVariable[] sortByBuffer(VariableIndexer mapper, NamedShaderVariable[] activeVariables, int index) {
    ...
    /**
     * Set the attribute pointer(s) using the data in the vertexbuffer, this shall
     * make the necessary calls to set the pointers for used attributes, enable
     * pointers as needed. This will make the actual connection between the
     * attribute data in the vertex buffer and the shader. It is up to the caller to
     * make sure that the attribute array(s) in the mesh contains valid data.
     * 
     * @param gles
     * @param mesh
     */
    ...
    /**
     * Sets the uniform data into the block - if BlockBuffer is dirty the UBO is
     * updated.
     * 
     * @param gles
     * @param block
    ...
    protected void setUniformBlock(GLES30Wrapper gles, BlockBuffer blockBuffer, ShaderVariable variable)
        ...
        if (blockBuffer.isDirty()) {
            ...
            /**
             * TODO - Another solution is to use glBufferSubData - but the benefit may not
             * be obvious since reusing the same buffer with new content may trigger wait
             * for buffer to be finished in rendering. Theoretically GL should handle
             * allocation of buffers in an optimized manner, effectively reusing a discarded
             * buffer.
             */
            gles.glBufferData(GLES30.GL_UNIFORM_BUFFER, blockBuffer.getSizeInBytes(),
                    blockBuffer.getBuffer().position(0), GLES30.GL_STATIC_DRAW);
            ...
            gles.glBindBufferBase(GLES30.GL_UNIFORM_BUFFER, vars.blockIndex, blockBuffer.getBufferName());
            GLUtils.handleError(gles, "setUniformBlock " + blockBuffer.getBlockName());
        }
    }

    /**
     * Uploads the uniforms to render backend When this method returns the uniform
     * data has been uploaded to GL and is ready.
     * 
     * @param gles
     */
    public void uploadUniforms(GLES20Wrapper gles, GraphicsShader shader) throws GLException {
        uploadUniforms(gles, shader.getUniformData(), activeUniforms);
    }

    /**
     * Uploads uniforms to GL, float array data is uploaded and if blockbuffer is
     * used it is bound. If uniform block buffer is dirty and uses UBO the data is
     * first uploaded to buffer object then uniform block is bound.
     * 
     * 
     * @param gles
     * @param uniformData
    ...
    /**
     * Uploads one of more float uniforms for the specified variable to GL, supports
     * VEC2, VEC3, VEC4 and MAT2, MAT3, MAT4 types
     * 
     * @param gles
    ...
    public final void uploadUniform(GLES20Wrapper gles, FloatBuffer uniforms, ShaderVariable variable)
        ...
        switch (variable.getDataType()) {
            case GLES20.GL_FLOAT:
                gles.glUniform1fv(variable.getLocation(), size, uniforms);
                break;
            case GLES20.GL_FLOAT_VEC2:
                gles.glUniform2fv(variable.getLocation(), size, uniforms);
                break;
            case GLES20.GL_FLOAT_VEC3:
                gles.glUniform3fv(variable.getLocation(), size, uniforms);
                break;
            case GLES20.GL_FLOAT_VEC4:
                gles.glUniform4fv(variable.getLocation(), size, uniforms);
                break;
            case GLES20.GL_FLOAT_MAT2:
                gles.glUniformMatrix2fv(variable.getLocation(), size, false, uniforms);
                break;
            case GLES20.GL_FLOAT_MAT3:
                gles.glUniformMatrix3fv(variable.getLocation(), size, false, uniforms);
                break;
        ...
        if (GLUtils.handleError(gles, "setUniform: " + variable.toString())) {
            /**
    ...
    /**
     * Prepares a texture used before rendering starts. This shall set texture
     * parameters to used textures, ie activate texture, bind texture then set
     * parameters. TODO - This should be moved to a class that handles nucleus
     * texture/mesh
     * 
     * @param renderer
     * @param texture
