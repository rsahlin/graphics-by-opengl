...
public class FPointerInputProcessorTest extends BaseTestCase implements MMIPointerInput {

    protected List<MMIPointer> pointerEvents = new ArrayList<MMIPointer>();
    ...
    public void testActionDownUpAll() {
        ...
        CoreInput processor = CoreInput.getInstance();
    ...
    private void assertActionPosition(List<MMIPointer> events, ArrayList<AssertMMIAction> assertValues,
        ...
        for (int i = 0; i < count; i++) {
            MMIPointer event = events.get(i + offset);
     ...
     * @param positions
     */
    private void createAndSend(PointerAction action, CoreInput processor, int count,
    ...
    private AssertMMIAction sendEvent(PointerAction action, CoreInput processor, int pointer,
    ...
    public void testActionDownMove() {
        ...
        processor.pointerEvent(PointerAction.DOWN, Type.FINGER, System.currentTimeMillis(),
                Pointer.POINTER_1, FIRST_POS, 0);
        processor.pointerEvent(PointerAction.MOVE, Type.FINGER, System.currentTimeMillis(), Pointer.POINTER_1,
                SECOND_POS, 0);

        Assert.assertEquals(2, pointerEvents.size());
        MMIPointer event = pointerEvents.get(0);
        Assert.assertEquals(com.nucleus.mmi.MMIPointer.Action.ACTIVE, event.getAction());
        event = pointerEvents.get(1);
        Assert.assertEquals(com.nucleus.mmi.MMIPointer.Action.MOVE, event.getAction());
        for (Pointer pointer : event.getPointerData().getPointers()) {
            Assert.assertEquals(Pointer.POINTER_1, pointer.pointer);
        }
    ...
    public void testMultipleDownMove() {
        ...
        checkList.add(sendEvent(PointerAction.DOWN, processor, Pointer.POINTER_1, positions, 0));
        checkList.add(sendEvent(PointerAction.MOVE, processor, Pointer.POINTER_1, positions, 2));
        checkList.add(sendEvent(PointerAction.MOVE, processor, Pointer.POINTER_1, positions, 4));
        checkList.add(sendEvent(PointerAction.MOVE, processor, Pointer.POINTER_1, positions, 6));
        checkList.add(sendEvent(PointerAction.UP, processor, Pointer.POINTER_1, positions, 8));

        assertActionPosition(pointerEvents, checkList, 0, checkList.size(), 0);
        pointerEvents.clear();
        checkList.clear();

        checkList.add(sendEvent(PointerAction.DOWN, processor, Pointer.POINTER_1, positions, 10));
        checkList.add(sendEvent(PointerAction.MOVE, processor, Pointer.POINTER_1, positions, 12));
        checkList.add(sendEvent(PointerAction.MOVE, processor, Pointer.POINTER_1, positions, 14));
        checkList.add(sendEvent(PointerAction.MOVE, processor, Pointer.POINTER_1, positions, 16));
        checkList.add(sendEvent(PointerAction.UP, processor, Pointer.POINTER_1, positions, 18));
    ...
    public void testManyActionDownUp() {
        ...
        float[] positions = createRandomPositions(10);
        checkList.add(sendEvent(PointerAction.DOWN, processor, Pointer.POINTER_1, positions, 0));
        checkList.add(sendEvent(PointerAction.DOWN, processor, Pointer.POINTER_2, positions, 2));
        checkList.add(sendEvent(PointerAction.UP, processor, Pointer.POINTER_1, positions, 4));
        checkList.add(sendEvent(PointerAction.DOWN, processor, Pointer.POINTER_1, positions, 6));
        checkList.add(sendEvent(PointerAction.DOWN, processor, Pointer.POINTER_3, positions, 8));
        checkList.add(sendEvent(PointerAction.UP, processor, Pointer.POINTER_2, positions, 10));
        checkList.add(sendEvent(PointerAction.DOWN, processor, Pointer.POINTER_2, positions, 12));
        checkList.add(sendEvent(PointerAction.DOWN, processor, Pointer.POINTER_4, positions, 14));
        checkList.add(sendEvent(PointerAction.DOWN, processor, Pointer.POINTER_5, positions, 16));
        checkList.add(sendEvent(PointerAction.UP, processor, Pointer.POINTER_1, positions, 18));
    ...
    public void testActionZoom() {
        ...
        processor.addMMIListener(this);
        processor.pointerEvent(PointerAction.DOWN, Type.FINGER, System.currentTimeMillis(), Pointer.POINTER_1,
                FINGER1_FIRST, 0);
        processor.pointerEvent(PointerAction.MOVE, Type.FINGER, System.currentTimeMillis(), Pointer.POINTER_1,
                FINGER1_SECOND, 0);
        processor.pointerEvent(PointerAction.DOWN, Type.FINGER, System.currentTimeMillis(), Pointer.POINTER_2,
                FINGER2_FIRST, 0);
        processor.pointerEvent(PointerAction.MOVE, Type.FINGER, System.currentTimeMillis(), Pointer.POINTER_2,
                FINGER2_SECOND, 0);
        ...
        Assert.assertEquals(com.nucleus.mmi.MMIPointer.Action.ZOOM, event.getAction());
    ...
    public void testActionZoomOutValues() {
        ...
        Iterator<MMIPointer> iterator = pointerEvents.iterator();
        int found = 0;
        while (found < LOOPCOUNT && iterator.hasNext()) {
            MMIPointer event = iterator.next();
     ...
     */
    private void createEvents(CoreInput processor, PointerAction action, float[] values, int count) {
        ...
        for (int i = 1; i < count + 1; i++) {
            ...
            processor.pointerEvent(action, Type.FINGER, i, Pointer.POINTER_1, pos1, 0);
            processor.pointerEvent(action, Type.FINGER, i, Pointer.POINTER_2, pos2, 0);
        }
